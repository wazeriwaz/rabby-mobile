...
interface Props {
  actionData: NonNullable<ParsedActionData['approveNFT']>;
}

export const RevokeNFTBtn = ({ actionData }: Props) => {
  const { t } = useTranslation();
  const { navigation } = useSafeSetNavigationOptions();
  const { styles, colors2024 } = useTheme2024({ getStyle });
  // todo erc 1155
  const { data: isApproved } = useRequest(async () => {
    return getErc721Approved({
      chainServerId: actionData.nft.chain,
      nftTokenId: actionData.nft.id,
      contractAddress: actionData.nft.contract_id,
    });
  });
        ...
        <Button
          ...
          onPress={async () => {
            await revokeNFTApprove({
              ...
              contractId: actionData.nft.contract_id,
              abi: 'ERC721',
